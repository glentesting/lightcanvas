...
Updated by: Claude (Parallel bug-hunt + fix sweep — 10 CRITICAL, multiple HIGH/MED across security, exports, audio, AI, state, UI)
...
- 2026-05-27 — Parallel bug-hunt + fix sweep across the whole codebase (6 audit agents, 6 fix agents). Landed: **Security** — bucket allowlist on service-role storage ops (DELETE + signed URL) to close arbitrary-object-delete / arbitrary-signed-URL holes; audio + photo upload size + MIME allowlists (SVG rejected to prevent stored XSS); autosave + import Zod schemas tightened with body-size caps; songs bucket RLS migration (005); share page proper auth via `share_token` column + opt-in publish flow (migration 006). **AI** — Anthropic model id `claude-sonnet-4-5-20250514` → `claude-sonnet-4-6`; prompt-injection mitigation via XML-delimited user input; AbortSignal plumbed through with 60s timeout; static prompt content moved to `system` array with `cache_control` for caching; mock-fallback now warns in all envs. **Exports** — ZIP UTF-8 filename flag (bit 11) set on both local + central headers; zip64 fail-fast guard; xLights/LOR timing rounding stabilised; audioFilename uses bare basename; hexToXl validates input; Content-Disposition + ZIP entry name sanitisation. **Audio** — WaveSurfer destroy race properly deferred with one-shot ready/error + 10s safety; OfflineAudioContext resample drift fixed by decoding at native sample rate; onset timestamp centred; BPM histogram octave bias removed; adaptive flux threshold matches comment. **State** — autosave selector uses shallow equality (was thrashing on every store change); AudioAnalysis + project name excluded from zundo history; undo stack cleared on loadProject; saved→idle timer cleaned up on unmount; multi-track drag preserves relative offsets; zoom clamped. **UI** — designer client redirect uses `router.replace` in useEffect; settings page hydrates from async Clerk user via effect; Timeline drop position uses pointer not delta.x and adds KeyboardSensor for a11y; LayoutEditor de-nested interactive elements; MobileGate listens for resize; PreviewPanel memoised; dashboard projects array-validated. Build clean, tsc clean, lint 0/0. 43 source files + 2 new migrations.
