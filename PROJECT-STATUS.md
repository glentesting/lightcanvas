...
- **AI sequencer (redesigned 2026-08-27): two-layer plan+expand pipeline** (`src/lib/ai/sequencer/`). Layer 1 (model `claude-opus-5`, direct fetch, one call per batch of ≤4 song sections) returns compact musical-direction plans — energy, active fixture groups, effect/palette/rhythm/movement, transitions — fed by the FULL analysis (per-section beat counts, loudness, onset density, spectral means; no more 20-beat truncation). Layer 2 (deterministic code) expands plans into beat-snapped effect blocks: chases, staggers, phrase-split beds, transitions — every block start is a real detected beat. Measured density (live `claude-opus-5`, real 5.4-min MP3, 20 fixtures): **1,907 blocks**, 100% starting exactly on detected beats, genuinely musical plans (quiet intro, layered choruses, transitions); the deterministic mock reaches 2,925 on the same input (old pipeline: ~60–100). Truncation (`stop_reason: max_tokens`), refusal, and partial-JSON salvage all handled explicitly. No silent mock: missing `ANTHROPIC_API_KEY` fails loudly (503); `AI_USE_MOCK=1` enables an explicit, UI-labeled deterministic mock. 5 style presets and refine prompts kept (a refine pass replaces the previous generation). See AI-PIPELINE-STATUS.md.
...
**ANTHROPIC_API_KEY set locally and verified live (2026-08-27); Vercel prod still unconfirmed.** The app fails loudly without it (503 + clear message) — no silent mock. A live `claude-opus-5` verification run produced 1,907 beat-snapped blocks (see AI-PIPELINE-STATUS.md). Still to do: set the key in Vercel prod and bump the generate route's `maxDuration` (planning takes ~60 s of sequential model calls).
...
- 2026-08-27 — Live Opus 5 verification of the AI sequencer: key set in .env.local (stripped invisible U+200B chars from the pasted line — they broke env parsing), verify script auto-uses the real planner. Result: 1,907 blocks / 20 fixtures / 100% beat-aligned / 4 clean API calls / zero-violation .loredit export. First live run was 1,338 (under target) — traveling chases swept one fixture per beat; expander now moves a ⌈n/4⌉ wave-front per pulse (sweep ≈ once per bar) and the prompt asks choruses for 3–5 layers. Mock on same input: 2,925.
