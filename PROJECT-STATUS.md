...
Updated by: Claude (Mobile QR house-photo upload)
...
- 2026-05-27 — QR upload security fixes (migration 006): tokens are now stored as SHA-256 hashes only (`upload_sessions.token` dropped, `token_hash` added). The plaintext bearer token only exists in the response to the desktop session-creation call and in the QR URL — even a full anon read of the table now leaks nothing useful. Also tightened mobile upload MIME validation: was `!allowlist.has(t) && !t.startsWith("image/")` which let `image/gif` and other arbitrary image types through; now uses the allowlist as the hard gate.
- 2026-05-27 — Mobile QR house-photo upload: new `upload_sessions` table (migration 005) with Supabase Realtime publication, `POST /api/upload-sessions` to mint a 15-min token-scoped session, public `GET/POST /api/mobile-upload/[token]` for phone uploads (validates token + expiry + size + type, rejects HEIC with friendly hint), public `/mobile-upload/[token]` page with Take Photo / Choose from Library buttons, and a new `HousePhotoUploadModal` on the Layout Editor with "From computer" / "From phone (QR)" tabs. Desktop subscribes to the session row via Realtime so the editor refreshes the instant the phone upload completes. Added `qrcode.react` dep. Middleware updated to make mobile routes public.
