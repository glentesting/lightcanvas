// Hero house — animated stylized illustration for the landing page.
// Each prop runs a DIFFERENT effect, so the preview reads as a real
// multi-track show, not one chase pattern repeated everywhere.
//
//   Roofline       → color-fade wash (chromatic shift, all lit)
//   Mega tree      → twinkle/sparkle (random pixels glitter)
//   Mini tree      → solid breathe (warm white, all pulse together)
//   Arch 1 (L)     → chase L→R
//   Arch 2 (mid)   → pulse-on-beat (all flash together)
//   Arch 3 (R)     → rainbow gradient sweep
//   Bushes L       → red/green random twinkle
//   Bushes R       → cool blue random twinkle
...
const HeroHouse = () => {
  ...
  const BPS = 142 / 60;
  const beatPulse = Math.max(0, 1 - ((t * BPS) % 1) * 1.4); // sharp attack, decay
  ...
  // Always lit, but a brightness peak walks left→right so it doesn't read as static.
  const roofline = [];
  const RFL_COUNT = 28;
  for (let i = 0; i < RFL_COUNT; i++) {
    ...
    // Brightness wave traveling across pixels
  ...
  ARCH_POSITIONS.forEach((a, ai) => {
    ...
    for (let i = 0; i < dots; i++) {
      ...
      if (a.mode === 'chase') {
        ...
        // Smooth sine wave of brightness — gentle, not a strobe
  ...
  // Positioned UNDER the windows, not flanking the house.
  const BUSH_CLUSTERS = [
    { cx: 250, cy: 312, count: 7, palette: 'rg' },   // under left windows
    { cx: 450, cy: 312, count: 7, palette: 'blue' }, // under right windows
  ];

  const winGlow = 0.4 + 0.4 * Math.sin(t * 0.8);
  const blink = (Math.floor(t * 0.5) % 6 === 0) && ((t * 2) % 1) < 0.18;

  // SINGING MOUTH — three states cycling on syllable rhythm.
  // Pattern: lyric phrasing — open words held, then closed, then open again.
  // Use a layered sin so it varies (not a perfect metronome).
  const phraseT = t * 1.8;
  ...
  return (
    <svg viewBox="0 0 720 420" style={{ width: '100%', height: '100%', display: 'block' }}>
      ...
      {[[80,40],[160,55],[240,30],[330,65],[420,45],[600,60],[670,40]].map(([x,y],i)=>(
        <circle key={'s'+i} cx={x} cy={y} r=".9" fill="#fff" opacity={.4+.4*Math.sin(t*1.5+i)}/>
      ))}
      ...
      <g transform={`translate(615 152) scale(${1 + 0.08*Math.sin(t*4)})`}>
        <path d="M 0 -8 L 2 -2 L 8 -2 L 3 2 L 5 8 L 0 4 L -5 8 L -3 2 L -8 -2 L -2 -2 Z"
              fill="#ffd76a" opacity=".95"/>
      </g>
      {/* Tree pixels — random twinkle */}
      {tree.map((p, i) => (
        <g key={'t'+i}>
          {p.intensity > .55 && <circle cx={p.x} cy={p.y} r="6" fill="url(#hh-glow)" opacity={p.intensity*.85}/>}
          <circle cx={p.x} cy={p.y} r="2.2"
            fill={`oklch(${65 + p.intensity*20}% 0.20 ${p.hue})`}
            opacity={.35 + p.intensity * .65}/>
        </g>
      ))}
      ...
      <g transform="translate(85 314) scale(1.18)">
        ...
        <path d="M 0 -34 L 1.5 -30 L 5.5 -30 L 2.2 -27.5 L 3.5 -23.5 L 0 -26 L -3.5 -23.5 L -2.2 -27.5 L -5.5 -30 L -1.5 -30 Z"
              fill="#ffd76a"/>
        ...
        {blink ? (
          ...
          <>
            ...
            <circle cx="4.5" cy="0" r="1.8" fill="#fff"/>
            <circle cx="-4" cy={.4 + mouthOpen * .3} r=".8" fill="#1a1410"/>
            <circle cx="5" cy={.4 + mouthOpen * .3} r=".8" fill="#1a1410"/>
          </>
        )}
        {/* SINGING MOUTH — opens and closes to the beat. Switches between
            shapes: closed smile, open "oh", wide "ah". */}
        {mouthOpen < 0.25 ? (
          // closed smile
          ...
          // wide "ah" — bigger oval, tongue visible
          <g>
            <ellipse cx="0" cy={9} rx={4 + mouthOpen} ry={3 + mouthOpen * 2.5} fill="#1a1410" stroke="#fff" strokeWidth="1.2"/>
        ...
        {Array.from({length: 8}).map((_, i) => {
          ...
          return (
            <g key={'mtl'+i}>
              {miniBreathe > .65 && <circle cx={tx} cy={ty} r="5" fill="url(#hh-glow)" opacity={miniBreathe*.6}/>}
      ...
      {roofline.map((p, i) => (
        <g key={'r'+i}>
          <circle cx={p.x} cy={p.y} r={9 + p.intensity * 4} fill="url(#hh-glow)" opacity={.4 + p.intensity * .4}/>
          <circle cx={p.x} cy={p.y} r="2.8"
            fill={`oklch(${68 + p.intensity*15}% 0.18 ${p.hue})`}/>
        </g>
      ))}
      ...
      {ARCH_POSITIONS.map((a, ai) => (
        <path key={'arch-bg-'+ai}
              ...
              fill="none" stroke="#5a4a32" strokeWidth="2" opacity=".9"/>
      ))}
      {archDots.map((p, i) => (
        <g key={'arch-'+i}>
          {p.intensity > .4 && <circle cx={p.x} cy={p.y} r="11" fill="url(#hh-glow)" opacity={p.intensity*.85}/>}
          <circle cx={p.x} cy={p.y} r="2.8"
            fill={`oklch(${68 + p.intensity*18}% 0.18 ${p.hue})`}
            opacity={.4 + p.intensity * .6}/>
        </g>
      ))}
        ...
        <g key={'bc'+ci}>
          ...
          {Array.from({length: cluster.count}).map((_, i) => {
            ...
            if (cluster.palette === 'rg') {
              ...
              hue = 220 + (i % 3) * 20; // cool blues
            }
            return (
              <g key={'bd'+ci+'-'+i}>
                {tw > .45 && <circle cx={x} cy={y} r="6" fill="url(#hh-glow)" opacity={tw*.7}/>}
                <circle cx={x} cy={y} r="2.2"
                  fill={`oklch(78% 0.18 ${hue})`} opacity={.35 + tw*.65}/>
              </g>
            );
          })}
        </g>
